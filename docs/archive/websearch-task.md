...
- [x] 1. Скопировать `services/search.ts` из `archive/web-search` + добавить `fast-xml-parser` в `server/package.json` + `YC_SEARCH_API_KEY` в `.env.example`. **Изменение от плана:** переделал `webSearch()` так, чтобы при ошибке (HTTP fail, parse fail, missing config) она throw'ила, а пустой массив возвращала только при реально пустом ответе. Это позволяет в роуте отличать `status: 'error'` от `status: 'success'` с пустыми sources.
- [x] 2. Миграция БД — колонка `tool_data TEXT` в `messages` (ALTER через PRAGMA, идемпотентно).
- [x] 3. `shared/types.ts` — `Source`, `MessageToolData`, тип `'tool'` в `SSEEvent`, поле `Message.toolData`. `done`-эвент дополнен `fullTool`.
- [x] 4. Бэк — `streamBodySchema.webSearch`, вызов `webSearch()` до LLM, два SSE-эвента (`loading` → `success`/`error`), `buildSearchPromptBlock` дописывается в system, insert+done. GET-роут парсит `tool_data` (JSON.parse).
- [x] 5. Фронт — `settingsStore.webSearch` (персистент) + `streamStore.partialTool: ToolState` + `onTool` callback с тремя статусами.
