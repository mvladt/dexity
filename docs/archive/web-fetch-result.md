...
- **Плашка Fetch = URL, без разворота** — показываем просто URL, который фетчится (на success — кликабельный, иначе текст; статус рисует сам ToolMessage). Минимум. `title` пока хранится в `toolData`, но в UI не отображается (кандидат на удаление).
