...
- **Плашка Fetch = домен, без разворота** — показываем домен фетчимой страницы (полный URL бывает длинным — captcha, query). На success домен кликабелен и ведёт на точный URL; статус рисует сам ToolMessage. `title` пока хранится в `toolData`, но в UI не отображается (кандидат на удаление).
