...
- [x] 10. **`ThinkingMessage`** — collapsible «думаю…» до первого токена стрима (TTFB Yandex 2-3 сек, UI кажется висящим). _Сделано: используем нативный `status='submitted'` у `MessageList` — aikit сам рисует `<Loader>` внизу, пока стрим запущен, но `partialContent/Thinking/Tool` ещё пусты. Как только пошёл первый delta — статус переключается на `'streaming'`, лоадер сменяется живым контентом. Свой `ThinkingMessage`-стаб писать не пришлось._
...
- [x] 20. **Подсветка кода + copy-button на блоках** — `MarkdownRenderer` на `highlight.js`. _Сделано из коробки: `@diplodoc/transform` подтягивает `highlight.js` и стили `yfm-code-floating-container`/`yfm-code-button` (copy-кнопка показывается по ховеру). Никакого кода не понадобилось — только проверить и удостовериться._
