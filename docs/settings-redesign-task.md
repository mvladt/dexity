...
- [x] ~~Секция «Интерфейс» — Web Search (есть в `settingsStore.webSearch`)~~ — 2026-07-03: тоггл убран из UI, веб-поиск включён всегда (`webSearch: true` хардкодом в `streamStore.ts`). Решение временное («пока»), см. `MEMORY.md`.
...
- [x] ~~Стриминг ответов — локальный `useState`, без сохранения. Бэк всегда отдаёт SSE.~~ — 2026-07-03: тоггл удалён из UI, поведение (SSE всегда) осталось как есть.
- [x] ~~Цитаты-маркеры — локальный `useState`. У нас всегда показываются.~~ — 2026-07-03: тоггл удалён из UI, маркеры `[N]` остаются всегда включены.
