...
export async function webSearch(
  ...
  if (!apiKey) throw new Error('YC_SEARCH_API_KEY is not set');
  ...
  if (!response.ok) {
    ...
    throw new Error(`Yandex Search HTTP ${response.status}: ${text.slice(0, 200)}`);
  }

  const json = (await response.json()) as { rawData?: string };
  if (!json.rawData) throw new Error('Yandex Search: response has no rawData');
  ...
  const parsed = parser.parse(cleanedXml) as Record<string, unknown>;

  const errorNode = (parsed as any)?.yandexsearch?.response?.error;
  if (errorNode) throw new Error(`Yandex Search API error: ${asText(errorNode)}`);

  const groups = (parsed as any)?.yandexsearch?.response?.results?.grouping?.group ?? [];
